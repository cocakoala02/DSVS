...
func (p *DataCollectionProtocol) GenerateData() (libdrynx.ResponseDPBytes, error) {
	...
	// clm, err = libdrynxencoding.GetDataFromDataProviderV4(p.Survey.Query.SQL, p.Survey.Query.Operation.NameOp, p.Survey.Query.FixedScale) //only return one/two Value for sum,count/mean
	clm, err = libdrynxencoding.GetDataFromDataProviderV5(p.Survey.Query.SQL, p.Survey.Query.Operation.NameOp, p.Survey.Query.FixedScale, p.Survey.Query.FloatColumns)
