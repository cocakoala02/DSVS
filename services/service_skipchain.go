...
func (s *ServiceDrynx) HandleSurveyQueryToVN(recq *libdrynx.SurveyQueryToVN) (network.Message, error) {
	...
	if s.ServerIdentity().String() == recq.SQ.Query.RosterVNs.List[0].String() {
		go func() {
			...
			// 为本次查询建立 surveyID -> blockID 的映射（推荐做法）
			libdrynx.UpdateDB(s.DB, "mapping", recq.SQ.SurveyID, []byte(newSB.Hash))

			// 不再维护全局 LastSkipBlock，避免后续被误用
			s.LastSkipBlock = nil
			s.Mutex.Unlock()
...
func (s *ServiceDrynx) verifyFuncBitmap(newID []byte, newSB *skipchain.SkipBlock) bool {
	...
	_, msg, err := network.Unmarshal(newSB.Data, libunlynx.SuiTe)
	if err != nil {
		log.Error("unmarshal block data:", err)
		return false
	}
	blockData := msg.(*libdrynx.DataBlock)
	...
	if s.DB == nil {
		db, err := OpenDB(s.DBPath)
		if err != nil {
			log.Error("open db:", err)
			return false
		}
		s.DB = db
	}

	bitMapFromServ := map[string]int64{}

	// ⚠️ 这里不要再 Fatal；bucket/键缺失视为校验失败即可
	_ = s.DB.View(func(tx *bbolt.Tx) error {
		b := tx.Bucket([]byte(s.ServerIdentity().Address.String()))
		if b == nil {
			log.Lvl2("verify: no bucket for", s.ServerIdentity().Address.String())
			return nil
		}
		v := b.Get([]byte(blockData.SurveyID + "/map"))
		if len(v) == 0 {
			log.Lvl2("verify: no bitmap for survey", blockData.SurveyID)
			return nil
		}
		_, m, err := network.Unmarshal(v, libunlynx.SuiTe)
		if err != nil {
			log.Error("unmarshal local bitmap:", err)
			return nil
		}
		bitMapFromServ = m.(*libdrynx.BitMap).BitMap
		return nil
	})

	// 做对比（本地没有就直接“失败”，但不崩）
	for k, v := range bitMapFromServ {
		if blockData.Proofs[k] != v {
			log.Lvl2("verify: mismatch", k, blockData.Proofs[k], "!=", v)
			return false
		}
	}
	return true
