...
func ComputeEncryptedApproxCoefficients(approxCoefficients [][]int64, pubKey kyber.Point) ([]*libunlynx.CipherVector, [][]kyber.Scalar) {
	...
	for j := 0; j < k; j++ {
		go func(j int) {
			defer wg.Done()
			for i := 0; i < len(approxCoefficients[j]); i++ {
				v := approxCoefficients[j][i]
				if v > libunlynx.MaxHomomorphicInt || v < -libunlynx.MaxHomomorphicInt {
					log.Fatalf("Error: %d exceeds ±%d", v, libunlynx.MaxHomomorphicInt)
				}
			}
			tmpCv, tmpRs := libunlynx.EncryptIntVectorGetRs(pubKey, approxCoefficients[j])
			encryptedApproxCoefficients[j] = tmpCv
...
func GetDataFromDataProvider(dpname string, DptoPath map[string]string, tablename string, sql_survey string, opname string) ([]int64, error) {
	...
	path := DptoPath[dpname]
	path = path + "/" + tablename + ".csv"
	// 原始 SQL 模板（用 %s 占位）
	// 	tpl := `SELECT SUM("明细项目单价")
	//    FROM read_csv_auto('%s', delim=',')`

	sq := sql_survey
	filepath := path
	if opname == "count" {
		// 2. 替换 FROM 后的表为 read_csv_auto(...)
		reFrom := regexp.MustCompile(`(?i)FROM\s+\w+`)
		sq = reFrom.ReplaceAllString(sq, fmt.Sprintf("FROM read_csv_auto('%s', delim=',')", filepath))
	...
	// 1. 提取字段名（SUM、AVG、等函数内）并转换为 SELECT field
	reFunc := regexp.MustCompile(`(?i)SELECT\s+(SUM|AVG|COUNT|MIN|MAX)?\s*\(\s*(\w+)\s*\)`)
	sq = reFunc.ReplaceAllString(sq, `SELECT $2`)

	// 2. 替换 FROM 后的表为 read_csv_auto(...)
	reFrom := regexp.MustCompile(`(?i)FROM\s+\w+`)
	sq = reFrom.ReplaceAllString(sq, fmt.Sprintf("FROM read_csv_auto('%s', delim=',')", filepath))

	// // 3. 为 WHERE 子句中的字段加引号（字段名 =、>=、<=、!=、<>、LIKE 等）
	// reWhereField := regexp.MustCompile(`(?i)(WHERE|AND|OR)\s+(\w+)\s*([=!<>]+|LIKE)`)
	// sq = reWhereField.ReplaceAllString(sq, `$1 "$2" $3`)
