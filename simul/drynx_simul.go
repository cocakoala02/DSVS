...
func NewSimulationDrynx(config string) (onet.Simulation, error) {
	...
	println("nnnnnnnnnnnsssssssssssslllllllllllddddddddddddddd")
	return sl, nil
}
...
func (sim *SimulationDrynx) Setup(dir string, hosts []string) (*onet.SimulationConfig, error) {
	...
	log.Lvl2("Setup done")
	println("ssssssssstttttttttttteeeeeeeeeeeeuuuuuuupppppppppp")
	return sc, nil
}
...
func (sim *SimulationDrynx) Run(config *onet.SimulationConfig) error {
	...
	if len(elVNs) > 0 {
		...
		if sim.Proofs != 0 {
			// proofs, err := clientSkip.SendGetProofs(rosterVNs.List[0], surveyID)
			// if err != nil {
			// 	log.Fatal("Error retrieving proofs:", err)
			// }
			// log.Lvl1("Retrieved Verification Proofs:")
			// for key, proofBytes := range proofs {
			// 	log.Lvl1(key, ": ", fmt.Sprintf("%x", proofBytes))
			// }
			libunlynx.EndParallelize(wg)
			// close DB
